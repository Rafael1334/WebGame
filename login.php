<?php
  ...
  if (!$login && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['username'])){
      ...
      $error = true;
      $error_msg_user = "Nome de usuário inválido";
    }
    if(isset($_POST['senha'])){
      ...
      $error = true;
      $error_msg_senha = "Senha inválida";
    }
   
    //checar emails iguais antes de continuar
    // mensagens de erro não funcionam, 
    if(!$error){
      $sql = "SELECT cliente.user_password, cliente.user_name, cliente.id, cliente.email FROM cliente WHERE cliente.user_name = '$username'";
      $result = mysqli_query($conn, $sql);
      if($result){
        if(mysqli_num_rows($result) == 1){
          $row = mysqli_fetch_assoc($result);
          $confirmasenha = $row['user_password'];
          $userid = $row['id'];
          if(!password_verify($senha,$confirmasenha)){
            $error = true;
            $error_msg_senha = "A senha ou nome de usuário está errado";
          }
          else{
            $sql = "UPDATE cliente set last_login_at = now() where id = '$userid'";
            $_SESSION["user_id"] = $row["id"];
            $_SESSION["user_name"] = $row["user_name"];
            $_SESSION["user_email"] = $row["email"];
            header("Location: " . dirname($_SERVER['SCRIPT_NAME']) . "/trabalhofinal.php");
            exit();
          ...
          $error = true;
          $error_msg_user = "Nome de usuário inexistente";
        }
      }        
    }
  }
    ...
    <main id="login">
      ...
      <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
        <div class="inputContainer">
          <input required="required" name="username" placeholder="Username" type="text">
          <?php if (!empty($error_msg_user)): ?>
            <span class="help-block"><?php echo $error_msg_user ?></span>
          <?php endif; ?>
        ...
        <label class="password-label">
          <!-- <input placeholder="Password" required="" pattern="^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)[A-Za-z\d]{8,}$" id = "senha" type="password"> -->
          <input placeholder="Password" required="" name="senha" type="password">
          <?php if (!empty($error_msg_senha)): ?>
            <span class="help-block"><?php echo $error_msg_senha ?></span>
          <?php endif; ?>
          ...
          <?php if ($login && $_SERVER["REQUEST_METHOD"] == "POST"): ?>
              <span class="help-block"><?php echo "Usuário já está logado" ?></span>
          <?php endif; ?>
